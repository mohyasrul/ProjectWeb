...
const UserDashboard = () => {
  ...
  const handleDeleteBooking = (bookingId: string) => {
    const updatedBookings = userBookings.filter((booking: any) => booking.id !== bookingId);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card className="max-w-md w-full">
          <CardContent className="p-6 text-center">
            <h2 className="text-xl font-semibold mb-4">Silakan masuk untuk melihat dashboard Anda</h2>
  ...
  const totalSpent = userBookings.reduce((sum: number, booking: any) => sum + booking.total, 0);
  const citiesVisited = new Set(userBookings.map((booking: any) => booking.location)).size;
